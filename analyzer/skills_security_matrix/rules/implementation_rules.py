...
def classify_implementation(evidence: list[EvidenceItem]) -> list[CategoryClassification]:
    ...
    atomic_decisions = build_atomic_decisions(evidence, layer="implementation", capability_mappings=definition.capability_mappings)
    candidates = build_rule_candidates(atomic_decisions, layer="implementation", matrix_by_id=matrix_by_id)
